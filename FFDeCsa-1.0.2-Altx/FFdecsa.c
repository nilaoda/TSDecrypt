...
// all lines containing DBG are filtered by the Makefile.
...
// possible choices
#define PARALLEL_032_4CHAR    320
#define PARALLEL_032_4CHARA   321
#define PARALLEL_032_INT      322
#define PARALLEL_064_8CHAR    640
#define PARALLEL_064_8CHARA   641
#define PARALLEL_064_2INT     642
#define PARALLEL_064_LONG     643
#define PARALLEL_064_MMX      644
...
#include "tmp_ffmode.h"
//#define PARALLEL_MODE PARALLEL_032_INT
#endif
//////// our choice //////////////// our choice //////////////// our choice //////////////// our choice ////////

#include "parallel_generic.h"
//// conditionals
#if PARALLEL_MODE==PARALLEL_032_4CHAR
#include "parallel_032_4char.h"
#elif PARALLEL_MODE==PARALLEL_032_4CHARA
#include "parallel_032_4charA.h"
#elif PARALLEL_MODE==PARALLEL_032_INT
#include "parallel_032_int.h"
#elif PARALLEL_MODE==PARALLEL_064_8CHAR
#include "parallel_064_8char.h"
#elif PARALLEL_MODE==PARALLEL_064_8CHARA
#include "parallel_064_8charA.h"
#elif PARALLEL_MODE==PARALLEL_064_2INT
#include "parallel_064_2int.h"
#elif PARALLEL_MODE==PARALLEL_064_LONG
#include "parallel_064_long.h"
#elif PARALLEL_MODE==PARALLEL_064_MMX
...
DBG  }
DBG  if(i%linelen==0) fprintf(stderr,"\n");
DBG }
...
void __stdcall DOEXP set_control_words(unsigned char *ev, unsigned char *od,csa_keys_set *myKeys){
	// csa_key_t tt; 
  // printf("%d\n", sizeof(csa_keys_set));
	// printf("%ld\n",(long)tt.iA-(long)&tt.ck);
	// printf("%ld\n",(long)tt.iB-(long)&tt.ck);
	// printf("%ld\n",(long)tt.ck_g-(long)&tt.ck);
	// printf("%ld\n",(long)tt.iA_g-(long)&tt.ck);
	// printf("%ld\n",(long)tt.iB_g-(long)&tt.ck);
	// printf("%ld\n",(long)tt.kk-(long)&tt.ck);
	// printf("%ld\n",(long)tt.kkmulti-(long)&tt.ck);
// could be made faster, but is not run often
  int bi,by;
  int i,j;
// key
  memcpy((*myKeys).even.ck,ev,8);
  memcpy((*myKeys).odd.ck,od,8);
// precalculations for stream
  key_schedule_stream((*myKeys).even.ck,(*myKeys).even.iA,(*myKeys).even.iB);
  key_schedule_stream((*myKeys).odd.ck,(*myKeys).odd.iA,(*myKeys).odd.iB);
  for(by=0;by<8;by++){
    for(bi=0;bi<8;bi++){
      (*myKeys).even.ck_g[by][bi]=((*myKeys).even.ck[by]&(1<<bi))?FF1():FF0();
      (*myKeys).odd.ck_g[by][bi]=((*myKeys).odd.ck[by]&(1<<bi))?FF1():FF0();
    }
  }
  for(by=0;by<8;by++){
    for(bi=0;bi<4;bi++){
      (*myKeys).even.iA_g[by][bi]=((*myKeys).even.iA[by]&(1<<bi))?FF1():FF0();
      (*myKeys).odd.iA_g[by][bi]=((*myKeys).odd.iA[by]&(1<<bi))?FF1():FF0();
      (*myKeys).even.iB_g[by][bi]=((*myKeys).even.iB[by]&(1<<bi))?FF1():FF0();
      (*myKeys).odd.iB_g[by][bi]=((*myKeys).odd.iB[by]&(1<<bi))?FF1():FF0();
    }
  }
// precalculations for block
  key_schedule_block((*myKeys).even.ck,(*myKeys).even.kk);
  key_schedule_block((*myKeys).odd.ck,(*myKeys).odd.kk);
  for(i=0;i<56;i++){
    for(j=0;j<BYTES_PER_BATCH;j++){
      *(((unsigned char *)&(*myKeys).even.kkmulti[i])+j)=(*myKeys).even.kk[i];
      *(((unsigned char *)&(*myKeys).odd.kkmulti[i])+j)=(*myKeys).odd.kk[i];
    }
  }
